...
public class ConfClass {
    ...
    private int aliveTimeGps;  //minute

    private int sleepTimeMeter;  //millisecond
    private int aliveTimeMeter;  //minute

    public ConfClass() {
        try {
            this.poolSize = 10;
            this.groupNum = 2;
            this.threadConf = new int[poolSize][5];
            this.sleepTimeGps = 1000;  //milliSeconds
            this.aliveTimeGps = 5;  //seconds

            this.sleepTimeMeter = 1000;  //milliSeconds
            this.aliveTimeMeter = 10;  //seconds
        } catch(Exception e) {
            e.printStackTrace();
        }
    }
    public ConfClass(int poolSize, int groupNum, int sleepTimeGps, int aliveTimeGps, int sleepTimeMeter, int aliveTimeMeter) {
        try {
            this.poolSize = poolSize;
            this.groupNum = groupNum;
            this.threadConf = new int[poolSize][5];
            this.sleepTimeGps = sleepTimeGps;
            this.aliveTimeGps = aliveTimeGps;

            this.sleepTimeMeter = sleepTimeMeter;
            this.aliveTimeMeter = aliveTimeMeter;
        } catch(Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void setThreadConf() {
        try {
            if (sleepTimeGps < 0 || aliveTimeGps < 0 || sleepTimeMeter < 0 || aliveTimeMeter < 0 || aliveTimeGps < aliveTimeMeter) {
            ...
            for (int i = 0; i < poolSize; ++i) {
                ...
                threadConf[i][1] = aliveTimeGps;  //thread alive time, 0 is forever, another is limited time
                threadConf[i][2] = sleepTimeMeter;
                threadConf[i][3] = aliveTimeMeter;
                threadConf[i][4] = setConfGroupNum(poolSize,groupNum,i);
            }
        } catch(Exception e) {
            e.printStackTrace();
        }
    ...
    public void setAliveTimeGps(int aliveTimeGps) {
        try {
            if (aliveTimeGps < 0) {
                throw new IllegalArgumentException();
            }
    ...
    public int getAliveTimeGps  () {
        System.out.println(this.aliveTimeGps);
        return this.aliveTimeGps;
    }
    public void setAliveTimeMeter(int aliveTimeMeter) {
        try {
            if (aliveTimeMeter < 0) {
                throw new IllegalArgumentException();
            }
    ...
    public int getAliveTimeMeter() {
        System.out.println(this.aliveTimeMeter);
        return this.aliveTimeMeter;
    }
